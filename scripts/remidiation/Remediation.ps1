...
Removes local user profiles not used in the last X days (default 90) and not currently loaded.  
Uses Remove-CimInstance on Win32_UserProfile (supported cleanup).  
#># ========================= # CONFIG # ========================= $StaleDays = 90 $MinProfileAgeDays = 14 $UserRoot = "C:\Users"$ExcludeProfileNames = @(
  "Default",
  "Default User",
  "Public",
  "All Users",
...
foreach ($item in $toRemove) {
  try {
    ...
    $failed += [pscustomobject]@{
      ...
      LastUseTime = $item.LastUseTime
      SID         = $item.SID
      Result      = "Failed"      Error       = $_.Exception.Message
    }
  }
}
Write-Output "`n=== Removal Summary ===" if ($removed.Count -gt 0) {
  Write-Output "Removed:"$removed | Format-Table -AutoSize | Out-String | Write-Output}if ($failed.Count -gt 0) {
  Write-Output "Failed:"$failed | Format-Table -AutoSize | Out-String | Write-Output# Exit non-zero so remediation run shows as failed/attention neededexit 1}
Write-Output "Remediation completed successfully." exit 0 